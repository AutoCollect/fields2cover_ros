...
namespace {
    ...
    double distance(const ToolpathGenerator::ToolPoint &a, const ToolpathGenerator::ToolPoint &b) {
        return std::hypot(b.x - a.x, b.y - a.y);
    }
}

// --------------------- Constructor & Setters ---------------------
ToolpathGenerator::ToolpathGenerator(int smooth_number, double toolpath_size, bool smooth_boundary)
...
void ToolpathGenerator::setContour(const ToolpathGenerator::ToolPolyline &contour) {
    contour_ = contour;
    if (contour_.empty())
...
// --------------------- Helper Functions ---------------------
double ToolpathGenerator::computeTotalLength(const ToolpathGenerator::ToolPolyline &points) const {
    double total = 0.0;
...
ToolpathGenerator::ToolPoint ToolpathGenerator::getPointFromOffset(double offset_val, const ToolpathGenerator::ToolPolyline &points) const {
    // Ensure the polyline is closed.
    ToolpathGenerator::ToolPolyline pts = points;
...
double ToolpathGenerator::findNearestParam(const ToolPoint &point, const ToolPolyline &points) const {
    if (points.size() < 2)
        return 0.0;
    ...
    for (size_t i = 0; i < points.size()-1; ++i) {
        ...
        if (seg_length < 1e-6)
            continue;
        double t = ((point.x - p.x) * (q.x - p.x) + (point.y - p.y) * (q.y - p.y)) / (seg_length * seg_length);
        if (t < 0.0) t = 0.0;
        if (t > 1.0) t = 1.0;
        ToolPoint proj;
...
std::optional<ToolpathGenerator::ToolPoint> ToolpathGenerator::lineSegmentIntersection(const ToolPoint &l0, const ToolPoint &l1,
...
double ToolpathGenerator::intersectParamWithLine(const ToolPolyline &points, const std::pair<ToolPoint, ToolPoint> &line) const {
...
std::pair<ToolpathGenerator::ToolPoint, ToolpathGenerator::ToolPoint> ToolpathGenerator::getRelatedLine(const ToolPoint &point, const ToolPolyline &points) const {
...
double ToolpathGenerator::advanceParameter(double d0, double toolpath_size, const ToolPolyline &points) const {
    double total_length = computeTotalLength(points);
...
double ToolpathGenerator::computeNextTurnParam(double d0, double toolpath_size, const ToolPolyline *points) const {
    if (points == nullptr)
        return std::min(d0 + toolpath_size * 0.01, 1.0);
    return advanceParameter(d0, toolpath_size, *points);
}

void ToolpathGenerator::blendPoints(ToolPolyline &current, const ToolPolyline &target) const {
...
std::vector<ToolpathGenerator::ToolPolyline> ToolpathGenerator::computeOffsets
    (double toolpath_size, const ToolpathGenerator::ToolPolyline &contour) const {

    // return results
    std::vector<ToolpathGenerator::ToolPolyline> offset_polygons;
    ...
    while (true)
    {
        ...
        const Clipper2Lib::Path64& currentPolygon = convertPathDtoPath64(allPolygons.back(), scale_);

        Clipper2Lib::ClipperOffset offsetter;
        offsetter.AddPath(currentPolygon, Clipper2Lib::JoinType::Round, Clipper2Lib::EndType::Polygon);
        ...
        ToolpathGenerator::ToolPolyline offsetPolyline;
...
void ToolpathGenerator::archimedeanSpiral() {
    ...
    for (size_t i = 0; i < offsets_.size(); ++i) {
        ...
        if (i != offsets_.size() - 1) {
            ...
            std::cout << "  Updating entry parameter from " << entry_d_0_ << " to " << new_d0 << "\n";
            entry_d_0_ = new_d0;
        }
    }
...
void ToolpathGenerator::archimedeanSpiralTrick() {
    ...
    for (size_t i = 0; i < offsets_.size() - 1; ++i) {
        ...
        entry_spiral_.insert(entry_spiral_.end(), path_segment.begin(), path_segment.end());
        entry_spiral_.push_back(entry_p1);
        entry_d_0_ = entry_d1;
    }
    double final_d1 = computeNextTurnParam(entry_d_0_, toolpath_size_, &offsets_.back());
    ToolPolyline final_segment = selectSubpolyline(offsets_.back(), entry_d_0_, final_d1);
    entry_spiral_.insert(entry_spiral_.end(), final_segment.begin(), final_segment.end());
...
void ToolpathGenerator::archimedeanSpiralSmooth() {
    ...
    for (size_t i = 0; i < offsets_.size() - 1; ++i) {
        ...
        entry_spiral_.insert(entry_spiral_.end(), current_half.begin(), current_half.end());
        ToolPoint entry_p0 = getPointFromOffset(entry_d_0_, offsets_[i]);
