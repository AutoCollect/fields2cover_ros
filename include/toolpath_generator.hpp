...
public:
    ...
    Clipper2Lib::Path64 convertPathDtoPath64(const Clipper2Lib::PathD& pathD, const double& scale) const {
    ...
    Clipper2Lib::PathD convertPath64toPathD(const Clipper2Lib::Path64& path64, const double& scale) const {
...
private:
    ...
    ToolPolyline selectSubpolyline(const ToolPolyline &points, double d0, double d1 = 1.0) const;
    ...
    double intersectParamWithLine(const ToolPolyline &points, const std::pair<ToolPoint, ToolPoint> &line) const;

    /// Computes a local tangent line at the point on the polyline nearest to 'point'.
    std::pair<ToolPoint, ToolPoint> getRelatedLine(const ToolPoint &point, const ToolPolyline &points) const;

    /// Advances the parameter along the polyline by a distance equal to toolpath_size.
    double advanceParameter(double d0, double toolpath_size, const ToolPolyline &points) const;

    /// Computes the next turning parameter along the polyline.
    double computeNextTurnParam(double d0, double toolpath_size, const ToolPolyline *points = nullptr) const;
    ...
    std::vector<ToolPolyline> computeOffsets(double toolpath_size, const ToolPolyline &contour) const;
};
